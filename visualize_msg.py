...
class MessageGraph:
    ...
    def plot(self, ax, adj, msgs, holds, truncate):
        lim = self.lim
        ax.clear()
        ax.axis('off')
    ...
    def plot_adj(self, adj, ax, truncate):
        ...
        for node in adj:
            ...
            label = node if truncate == - \
                1 else node[:truncate + len(node) - self.max_node_length]
            ax.text(c, s, label,
...
if __name__ == '__main__':
 ...
 # read the adjacency matrix JSON file
    ...
    enum = [(len(snapshots) - 1, snapshots[-1])
            ] if args.last else enumerate(snapshots)
    for i, t in enum:
        # snapshots: adj, msgs, holders
        adj = t['adj']
        msgs = t['msgs']
        holders = t['holders']
        graph.plot(ax, adj, msgs, holders, truncate=args.truncate)
